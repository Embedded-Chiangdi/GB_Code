...
int mail_alarm_file_check(char *alarm_dir,char *ip_buf,char *mail_address_buf);
int mail_alarm_send(char *warn_file,char *ip_buf,char *mail_address_buf);
...
int mail_alarm_file_check(char *alarm_dir,char *ip_buf,char *mail_address_buf)
{
    ...
    if((dir_ptr=opendir(alarm_dir)) == NULL)
    ...
    while((pdirent=readdir(dir_ptr)) != NULL) 
        {
            ...
            if(strcmp(file_name,".")==0 || strcmp(file_name,"..")==0)  
                continue;
            mail_alarm_send(file_name,ip_buf,mail_address_buf);
            //printf("%s\n",file_name);   
        }
            closedir(dir_ptr);
...
int mail_alarm_send(char *warn_file,char *ip_buf,char *mail_address_buf)
{
            ...
            sprintf(command,"cat "ALARM_DIR"/%s | mailx -v -s \"Alarm: %s IP: %s\" %s", warn_file,
                                                                            warn_file,
                                                                            ip_buf,
                                                                            mail_address_buf);
            printf("command is %s\n",command);
            ...
            sprintf(command,"rm -f "ALARM_DIR"/%s",warn_file);
            printf("command is %s\n",command);
            system(command);
